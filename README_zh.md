...
2. 传统工具调用会将完整结果返回到模型的上下文窗口。分析 20 名员工的费用意味着 2,000 多行数据会污染上下文——仅生成摘要就需要超过 110,000 个 token。使用 PTC，代码在沙箱中运行，在本地处理数据，只有最终输出返回给模型。结果：token 减少 85-98%。
